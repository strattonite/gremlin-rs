...
impl AnonymousTraversal {
    ...
    pub fn aggregate<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().aggregate(args)
    }

    pub fn and<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().and(args)
    }

    pub fn as_<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().as_(args)
    }

    pub fn barrier<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().barrier(args)
    }

    pub fn both<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().both(args)
    }

    pub fn bothE<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().bothE(args)
    }

    pub fn bothV<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn by<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().by(args)
    }

    pub fn call<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().call(args)
    }

    pub fn cap<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().cap(args)
    }

    pub fn choose<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().choose(args)
    }

    pub fn coalesce<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().coalesce(args)
    }

    pub fn coin<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().coin(args)
    }

    pub fn connectedComponent<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().connectedComponent(args)
    }

    pub fn constant<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn cyclicPath<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().cyclicPath(args)
    }

    pub fn dedup<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn elementMap<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().elementMap(args)
    }

    pub fn emit<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn filter<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn from<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().from(args)
    }

    pub fn group<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().group(args)
    }

    pub fn groupCount<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().groupCount(args)
    }

    pub fn has<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().has(args)
    }

    pub fn hasId<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().hasId(args)
    }

    pub fn hasKey<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().hasKey(args)
    }

    pub fn hasLabel<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().hasLabel(args)
    }

    pub fn hasNot<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().hasNot(args)
    }

    pub fn hasValue<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn in_<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().in_(args)
    }

    pub fn inE<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().inE(args)
    }

    pub fn inV<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn is<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn limit<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().limit(args)
    }

    pub fn local<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn map<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().map(args)
    }

    pub fn match_<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().match_(args)
    }

    pub fn math<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn mergeE<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().mergeE(args)
    }
    pub fn mergeV<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn not<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().not(args)
    }

    pub fn option<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().option(args)
    }
    pub fn or<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn otherV<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().otherV(args)
    }
    pub fn out<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().out(args)
    }
    pub fn outE<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().outE(args)
    }

    pub fn outV<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
    ...
    pub fn program<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().program(args)
    }
    pub fn project<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().project(args)
    }
    pub fn properties<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().properties(args)
    }
    pub fn property<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().property(args)
    }
    pub fn propertyMap<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().propertyMap(args)
    }
    pub fn range<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().range(args)
    }
    pub fn read<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().read(args)
    }
    pub fn repeat<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().repeat(args)
    }

    pub fn sack<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().sack(args)
    }
    pub fn sample<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().sample(args)
    }
    pub fn select<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().select(args)
    }
    pub fn shortestPath<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().shortestPath(args)
    }
    pub fn sideEffect<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().sideEffect(args)
    }
    pub fn simplePath<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().simplePath(args)
    }
    pub fn skip<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().skip(args)
    }
    pub fn store<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().store(args)
    }
    pub fn subgraph<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().subgraph(args)
    }
    pub fn sum(&self) -> Traversal {
        self.traversal.clone().sum()
    }
    pub fn tail<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().tail(args)
    }
    pub fn timeLimit<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().timeLimit(args)
    }
    pub fn times<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().times(args)
    }
    pub fn to<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().to(args)
    }
    pub fn toE<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().toE(args)
    }
    pub fn toV<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().toV(args)
    }
    pub fn tree<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().tree(args)
    }
    pub fn unfold(&self) -> Traversal {
        self.traversal.clone().unfold()
    }
    pub fn union<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().union(args)
    }
    pub fn until<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().until(args)
    }
    pub fn value(&self) -> Traversal {
        self.traversal.clone().value()
    }
    pub fn valueMap<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().valueMap(args)
    }

    pub fn values<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().values(args)
    }
    pub fn where_<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
        self.traversal.clone().where_(args)
    }
    pub fn with<T: Into<Step> + Clone>(&self, args: T) -> Traversal {
